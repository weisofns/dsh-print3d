#!/usr/bin/env node
/**
 * gcode_render.js — 零依赖 G-code 刀路可视化 → PNG（俯视图）。
 *
 * 用法:
 *   node gcode_render.js <file.gcode> [--out out.png] [--size 1000]
 *
...
function fail(msg) { console.error('gcode_render: ' + msg); process.exit(1); }
...
function render(segs, size) {
  ...
  const py = (Y) => Math.round((maxY - Y + pad) * scale); // 翻转 Y：打印坐标 Y 向上
...
function main() {
  ...
  if (!args.file) fail('用法: node gcode_render.js <file.gcode> [--out out.png] [--size 1000]');
