#!/usr/bin/env node
/**
 * gcode_estimate.js — 零依赖 G-code 分析器（Marlin/Klipper 方言）。
 *
 * 用法:
 *   node gcode_estimate.js <file.gcode> [--density <g/cm3>]
 *
 * 输出: JSON。打印时间按匀速进给估算（忽略加减速），耗材按 1.75mm 线径计算。
 */
'use strict';
...
function fail(msg) {
  console.error('gcode_estimate: ' + msg);
  process.exit(1);
}
...
function main() {
  ...
  if (!args.file) fail('用法: node gcode_estimate.js <file.gcode> [--density <g/cm3>]');
