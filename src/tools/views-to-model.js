...
export function makeViewsToModelTool(ctx) {
  return {
    ...
    description:
      'CAD 三视图 → 3D 模型：把正视图/俯视图/侧视图的剪影各自沿视线方向挤出后求交集（视觉凸包），' +
      '生成可打印的 STL。对棱柱类机械件可精确还原；带内部孔洞的复杂件会偏保守（孔可能被填上）。' +
      '三张视图需为高对比剪影图（深色物体/浅色底，或反之），且比例一致。默认写到 桌面/3Doutput/stl/。',
    parameters: {
      type: 'object',
      properties: {
        ...
        max_pixels: { type: 'number', description: '每张视图解码后最长边像素（默认 400）。' },
      ...
      required: ['front_path', 'top_path', 'side_path'],
    },
    output: {
      schema: { type: 'object' },
      render(_args, value) {
    ...
    async execute(args, exec) {
      for (const k of ['front_path', 'top_path', 'side_path']) {
        if (!existsSync(args[k])) throw new Error(`print3d_views_to_model: 视图不存在：${args[k]}`)
      }
