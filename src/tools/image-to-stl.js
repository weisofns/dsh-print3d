  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        depth_mm: { type: 'number', description: 'extrude 模式挤出高度/总厚度 mm（默认 5）。' },
        base_mm: { type: 'number', description: 'extrude 模式底板厚度 mm（默认 0=无底板，只出剪影；>0 时剪影凸起在整块底板上）。' },
        engrave: { type: 'boolean', description: '雕刻模式（凹字）：true 时把剪影从整块实心板中下凹 carve_mm（默认 false=凸字）。' },
        carve_mm: { type: 'number', description: 'engrave 模式的下凹深度 mm（默认 2）。' },
