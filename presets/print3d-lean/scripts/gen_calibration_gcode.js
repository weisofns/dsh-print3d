#!/usr/bin/env node
/**
 * gen_calibration_gcode.js — 零依赖校准件 G-code 生成器（Marlin/Klipper 方言）。
 *
 * 用法:
 ...
 *   node gen_calibration_gcode.js --part first-layer --size 60 --out firstlayer.gcode
...
function fail(msg) { console.error('gen_calibration_gcode: ' + msg); process.exit(1); }
...
function main() {
  ...
  const cfg = {
    nozzle: num(args.nozzle, 200, 0, 'nozzle'),
    bed: num(args.bed, 60, 0, 'bed'),
    layerHeight: num(args['layer-height'], 0.2, 0.05, 'layer-height'),
    lineWidth: num(args['line-width'], 0.4, 0.1, 'line-width'),
    speed: num(args.speed, 40, 1, 'speed'),
    size: num(args.size, part === 'first-layer' ? 60 : 20, 1, 'size'),
  ...
  console.log(`已生成 ${out}（${part}，${(text.length / 1024).toFixed(1)} KB）`);
